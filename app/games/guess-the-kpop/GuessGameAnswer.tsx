...
const GuessGameAnswer: React.FC<GuessGameAnswerProps> = ({ onSelectOption, selectedOptionId, option, isFinalRound }) => {
	if (isFinalRound) {
		return (
			<button className={option.correct ? "border-2 border-green-500 rounded-lg p-4 text-left bg-green-50 transition-colors duration-250" : selectedOptionId === option.videoId ? "border-2 border-red-500 bg-red-50 rounded-lg p-4 text-left transition-colors duration-250" : "border-2 border-gray-300 rounded-lg p-4 text-left transition-colors duration-250"}>
				{option.answer}
			</button>
		)
	}
